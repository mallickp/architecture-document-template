...
[TIP]
====
Give here the security level of the data center according to the Uptime Institute scale (Tier from 1 to 4).
...
It is often useful to subdivide environnements into 'platforms' made up of a set of technical components isolated from each other (although they may share common hardware resources). For example, a test environment can consist of lanes `UAT1` and` UAT2` allowing two testers to work in isolation.
...
[TIP]
====
List here the technical measures that meet the <<Purge requirements>>.
====
====
